...
export function transformHoldingsToSnapshot(holdings) {
  ...
  const totalValue = holdings.total_value || { amount: 0, currency: 'USD' }

  // Transform positions to holdings format
  const spotHoldings = positions.map(position => {
    const symbol = position.symbol?.symbol || 'UNKNOWN'
  ...
  return {
    ...
    total_portfolio_value: parseFloat(totalValue.amount || 0),
    total_spot_value: parseFloat(totalValue.amount || 0),
    total_futures_value: 0,
    holdings: spotHoldings,
    cash: totalCash,
    primary_currency: totalValue.currency || 'USD',
    account_type: 'SPOT',
    exchange: 'snaptrade',
  }
}
