...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if ((event === 'SIGNED_IN' || event === 'INITIAL_SESSION') && session?.user) {
        ...
        if (event === 'SIGNED_IN') {
          ...
          const isNewUser = !existingUser && event === 'SIGNED_IN'
    ...
    // OPTIMIZATION: Only call checkUser if we don't have an initial user
    // This avoids redundant API calls when localStorage already has valid session
    if (!hasInitialUser) {
      checkUser();
    } else {
      // Still verify the session is valid in the background, but don't block
      checkUser();
    }
