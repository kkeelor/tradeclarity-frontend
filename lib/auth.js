 ...
 * @param {string} redirectPath - Path to redirect to after successful login (default: '/dashboard')
 */
export async function signInWithGoogle(forceAccountSelection = false, redirectPath = '/dashboard') {
  try {
    // Get current path if redirectPath not provided
    const currentPath = typeof window !== 'undefined' ? window.location.pathname : '/dashboard';
    const finalRedirectPath = redirectPath || currentPath;
    
    // Ensure redirect path is safe (starts with / and doesn't start with //)
    const safeRedirectPath = finalRedirectPath.startsWith('/') && !finalRedirectPath.startsWith('//') 
      ? finalRedirectPath 
      : '/dashboard';
    
    // Build redirect URL with redirect path as query param
    const redirectUrl = new URL(`${window.location.origin}/auth/callback`);
    redirectUrl.searchParams.set('redirect', safeRedirectPath);
    
    console.log('🔵 Step 1: Starting Google signin...');
    console.log('🔵 Redirect URL will be:', redirectUrl.toString());
    console.log('🔵 Redirect path:', safeRedirectPath);
