 ...
 * Matches the logic from mcpClient.js
 * Dynamically detects all numbered keys (stops at first gap)
 * @returns {Array<number>} Array of key indices (0-based)
 */
function getAvailableAPIKeyIndices() {
  const indices = []
  
  // Check for numbered keys (ALPHA_VANTAGE_API_KEY_1, _2, _3, etc.)
  // Continue until we find a gap (no more consecutive keys)
  // Support up to 100 keys to prevent infinite loops, but stop at first gap
  let foundKeys = 0
  for (let i = 1; i <= 100; i++) {
    const key = process.env[`ALPHA_VANTAGE_API_KEY_${i}`]
    if (key) {
      // Key indices are 0-based, so key 1 = index 0, key 2 = index 1, etc.
      indices.push(i - 1)
      foundKeys++
    } else if (foundKeys > 0) {
      // We found some keys before, but this one is missing - stop here
      break
    }
    // If we haven't found any keys yet, keep looking
  }
  
  // If no numbered keys found, check for single fallback key
  if (indices.length === 0) {
    const singleKey = process.env.ALPHA_VANTAGE_API_KEY
    if (singleKey) {
      indices.push(0) // Single key uses index 0
    }
  }
 ...
 * Returns all available keys from env vars, even if they haven't been used yet
 * @returns {Promise<Array>} Array of { api_key_index, requests_today, last_reset }
 */
export async function getRequestsPerKeyToday() {
  try {
    const supabase = getSupabaseClient()
    
    // Get all available API key indices from environment variables
    const availableKeyIndices = getAvailableAPIKeyIndices()
    
    if (availableKeyIndices.length === 0) {
      console.warn('[MCP Analytics] No API keys found in environment variables')
      return []
    }

    // If no database, return keys with 0 usage
    if (!supabase) {
      return availableKeyIndices.map(index => ({
        api_key_index: index,
        requests_today: 0,
        last_reset: null
      }))
    ...
    if (error) {
      ...
      // Fallback: return available keys with 0 usage
      return availableKeyIndices.map(index => ({
        api_key_index: index,
        requests_today: 0,
        last_reset: null
    ...
    // Create a map of database entries by key index
    const dbEntries = new Map()
    if (data) {
      data.forEach(entry => {
        dbEntries.set(entry.api_key_index, entry)
      })
    }

    // Return all available keys, merging with database data
    return availableKeyIndices.map(index => {
      const dbEntry = dbEntries.get(index)
      if (dbEntry) {
        return {
          api_key_index: index,
          requests_today: dbEntry.requests_today || 0,
          last_reset: dbEntry.last_reset || null
        }
      } else {
        // Key exists in env but not in database (hasn't been used yet)
        return {
          api_key_index: index,
          requests_today: 0,
          last_reset: null
