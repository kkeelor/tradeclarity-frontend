 ...
 * 
 * ENHANCED VERSION with Dynamic Prompt Engine
 * 
 * Provides context-aware sample questions based on:
 * - User's actual trading data (specific symbols, win rates, P&L)
 * - Time context (time of day, recency of activity)
 * - Conversation history (what topics have been explored)
...
import { 
  ...
  getCoachModeStarters as getDynamicCoachStarters,
  generateGreeting,
  FALLBACK_NO_DATA
} from './dynamicPrompts'

// ============================================================================
// LEGACY QUESTION SETS (kept for backward compatibility)
// ============================================================================
...
const COACH_MODE_STARTERS = {
  ...
  education: [
    ...
    "Why do I take profits too early but hold losses too long?",
    "Am I trading with my head or my emotions?",
    "What's really costing me money?",
    "Am I overtrading or just active?",
    "Why do I feel confident after wins but scared after losses?",
    "What patterns am I missing in my own trading?",
    "Am I following a plan or just gambling?",
    "What would my trading look like if I removed emotion?"
  ]
}
 ...
 * 
 * ENHANCED: Now uses the Dynamic Prompt Engine for personalized questions
 * that include specific user data (symbols, win rates, etc.)
 * 
 * @param {Object} tradesStats - Trading statistics
 * @param {Object} analytics - Analytics data
 * @param {Array} previousConversations - Previous conversation summaries (optional)
 * @param {Object} options - Additional options
 * @returns {Array<string>} Array of sample questions
 */
export function getDynamicSampleQuestions(tradesStats, analytics, previousConversations = [], options = {}) {
  // No data case
  if (!tradesStats || tradesStats.totalTrades === 0) {
    return NO_DATA_QUESTIONS
  }
  
  // Try dynamic prompts first (with personalization)
  try {
    const ctx = buildPromptContext(tradesStats, analytics, previousConversations, options)
    const { prompts } = generateDynamicPrompts(ctx, { 
      maxPrompts: 8,
      includeGreeting: false,
      coachMode: options.coachMode || false
    })
    ...
    console.warn('Dynamic prompts failed, falling back to legacy:', e.message)
  }
  
  // Fallback to legacy behavior
  return getLegacySampleQuestions(tradesStats, analytics)
}

/**
 * Legacy sample questions (original behavior)
 * Kept for backward compatibility
 */
function getLegacySampleQuestions(tradesStats, analytics) {
  // No data case
  if (!tradesStats || tradesStats.totalTrades === 0) {
    return NO_DATA_QUESTIONS
  }
 ...
 * 
 * ENHANCED: Now uses the Dynamic Prompt Engine for contextual starters
 * 
 * @param {Object} tradesStats - Trading statistics
 * @param {Object} analytics - Analytics data
 * @param {Array} previousConversations - Previous conversation summaries (optional)
 ...
 */
export function getCoachModeStarters(tradesStats, analytics, previousConversations = []) {
  ...
  // Try dynamic coach starters first
  try {
    const ctx = buildPromptContext(tradesStats, analytics, previousConversations)
    const starters = getDynamicCoachStarters(ctx, 5)
    
    if (starters && starters.length > 0) {
      return starters
    }
  } catch (e) {
    console.warn('Dynamic coach starters failed, falling back to legacy:', e.message)
  }
  
  // Fallback to legacy behavior
  return getLegacyCoachModeStarters(tradesStats, analytics)
}

/**
 * Legacy coach mode starters (original behavior)
 */
function getLegacyCoachModeStarters(tradesStats, analytics) {
  // No data case - mix of onboarding and education
  if (!tradesStats || tradesStats.totalTrades === 0) {
    return [
      ...COACH_MODE_STARTERS.noData.slice(0, 3),
 ...
 * @param {Object} tradesStats - Trading statistics
 * @param {Object} analytics - Analytics data  
 * @param {Array} previousConversations - Previous conversation summaries
 * @returns {string|null} Greeting message or null
 */
export function getGreeting(tradesStats, analytics, previousConversations = []) {
  try {
    const ctx = buildPromptContext(tradesStats, analytics, previousConversations)
    return generateGreeting(ctx)
  } catch (e) {
    console.warn('Greeting generation failed:', e.message)
    return null
  }
}

// Re-export buildPromptContext for external use
export { buildPromptContext } from './dynamicPrompts'
